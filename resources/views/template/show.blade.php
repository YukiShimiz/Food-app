...
<body>
    ...
    <div class="ml-3 mt-3">
    <a type="button" class="btn btn-link" href="/food/management/template/index/{{Auth::User()->id}}">戻る</a>
    ...
    <form action="/food/management/template/edit/{{$template_title_body->id}}" method="POST" id="template_edit">
        @csrf
        <input type="button" class="btn btn-outline-primary mt-3 mb-3" onclick="return editpost(this);" value="編集を保存"/>
    <div class="form-group">
        <details>
            <summary>タイトル・説明</summary>
                <p>タイトル</p>
                <input type="textarea" class="form-control" name="template_title_body[title]" value="{{old('template_title_body.title',
                $template_title_body->title)}}"/><br>
                <p>説明</p>
                <input type="textarea" class="form-control" name="template_title_body[body]" value="{{old('template_title_body.body',
